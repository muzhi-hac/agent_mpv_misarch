...
每个 arm 现在都会输出一个 `metrics` 块(由共享计量器 `scripts/run_metrics.py` 在
公共 HTTP/LLM choke point `post_json` / `responses_api_call` 处埋点收集),这样延迟
可以被**分解**、成本可以跨架构对比,而不是只报一个不透明的总数。

| 指标 | Schema 字段名 | 含义 | 预期方向 |
|------|-------------|------|---------|
| 端到端延迟 | `duration_ms` | 全程毫秒数 | A < B < C |
| LLM 调用次数 | `metrics.llm_calls` | 模型往返次数(延迟的主要驱动) | B/D(ReAct 循环)> C(固定跳数)> A |
| 模型耗时 | `metrics.llm_ms` | 模型调用内的墙钟时间;`duration_ms − llm_ms` = 后端/协议耗时 | 跟随 `llm_calls` |
| Token 用量 | `metrics.{prompt,completion,total}_tokens` | 模型消耗的 token | 随 prompt 大小 + 调用次数增长 |
| 网络字节 | `metrics.{bytes_sent,bytes_recv}` | 线上字节(后端 + 模型两个 channel) | C 多了 Agent Card + task 跳 |
| HTTP 次数 | `metrics.http_calls` | 后端 + 模型请求数 | — |
| 客户端 CPU/内存 | `metrics.{cpu_seconds,peak_rss_mb}` | agent 主机上的编排开销(psutil 采样) | 相对 LLM 等待很小;B/D(ReAct)≥ C |
| 服务端工作量 | `metrics.server.total_alloc_bytes_delta` | Go 网关在该 task 期间分配的字节(单调 `TotalAlloc` 差值,任务前后读 `GET /debug/runtime-metrics`;低噪声、不受 GC 时机影响) | 随触及的协议层数增长 |

每次运行还会录制一份**统一对话文稿**——agent↔服务器的协议对话(A2A task、MCP tool
call、GraphQL)与 agent↔LLM 的 prompt/completion 按调用顺序交织在一起——写入
`result.transcript`,并另存一份可读的 `<output>.transcript.md` sidecar,这样一次运行
可以当作一条完整通信日志回放(适合做报告附录 / pipeline 图)。
